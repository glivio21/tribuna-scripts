javascript:(function(){
 ...
 // Categorias e scripts
  ...
  scripts: [
    ...
    ["Renomeador de Aldeias Lívio", "https://icons.iconarchive.com/icons/be-os/be-box/32/Be-IDE-icon.png", "javascript:(function( ){    /* Script criado por: Tribuna TW :) */    if(!window.location.href.includes(\"mode=prod\")){        if(confirm(\"Você não está na tela certa. Ir agora para a Visualização de Produção?\")){            window.location.href=\"game.php?screen=overview_villages&mode=prod\";        }        return;    }    if(document.getElementById(\"renameGui\")){ return; }    let gui=document.createElement(\"div\");    gui.id=\"renameGui\";    gui.style.position=\"fixed\";    gui.style.top=\"100px\";    gui.style.left=\"50%\";    gui.style.transform=\"translateX(-50%)\";    gui.style.background=\"#222\"; /* Alterado para tema escuro */    gui.style.padding=\"10px\"; /* Reduzido para janela menor */    gui.style.border=\"2px solid #666\"; /* Alterado para tema escuro */    gui.style.borderRadius=\"10px\";    gui.style.zIndex=9999;    gui.style.boxShadow=\"0px 0px 15px rgba(0,0,0,0.8)\"; /* Ajustado para tema escuro */    gui.innerHTML=`        <label style=\"color:#eee;\"><b>Nome da aldeia:</b></label>  \\n        <input id=\"villageBaseName\" type=\"text\" style=\"width:150px;margin:5px 0;background:#333;color:#eee;border:1px solid #555;\">  
        <label style=\"color:#eee;\"><input type=\"checkbox\" id=\"seqCheck\"> Adicionar numeração sequencial</label>  
        <label style=\"color:#eee;\"><b>Velocidade:</b></label>  
        <select id=\"speedSelect\" style=\"margin-top:5px;background:#333;color:#eee;border:1px solid #555;\">            <option value=\"300\">🐢 Devagar</option>            <option value=\"150\" selected>⚖️ Médio</option>            <option value=\"50\">⚡ Rápido</option>        </select>  
        <button id=\"startRename\" style=\"margin-top:10px;padding:5px 10px;background:#555;color:#eee;border:1px solid #777;\">▶️ START</button>        <button id=\"closeGui\" style=\"margin-left:10px;padding:5px 10px;background:#555;color:#eee;border:1px solid #777;\">❌ Fechar</button>        <div style=\"margin-top:10px;font-size:11px;color:#aaa;\">Tribuna Tribal Wars</div>    `; /* Cores do texto e inputs ajustadas para tema escuro, largura do input reduzida */    document.body.appendChild(gui);    document.getElementById(\"closeGui\").onclick=function(){ gui.remove(); };    document.getElementById(\"startRename\").onclick=function(){        let name=document.getElementById(\"villageBaseName\").value.trim();        let useSeq=document.getElementById(\"seqCheck\").checked;        let speed=parseInt(document.getElementById(\"speedSelect\").value);        if(!name||name.length<3){            alert(\"Nome deve ter ao menos 3 letras.\");            return;        }        let delay=0;        document.querySelectorAll(\"#production_table tr\").forEach((row,i)=>{            if(i===0)return;            setTimeout(()=>{                let icon=row.querySelector(\".rename-icon\");                if(icon)icon.click();                let input=row.querySelector(\".quickedit-edit input[type=text]\");                let btn=row.querySelector(\".quickedit-edit input[type=button]\");                if(input&&btn){                    input.value=name+(useSeq?(\" \"+String(i).padStart(2,\"\")):"\");                    btn.click();                }            }, delay+=speed);            delay+=speed;        });        gui.remove();    };})();"]
