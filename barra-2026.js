 ...
 // Categorias e scripts
  ...
  scripts: [
    ...
    },
    {
      ...
      scripts: [
       ["Calculadora de Snip + Aflição", "https://icons.iconarchive.com/icons/be-os/be-box/32/APPS-Stop-icon.png", `javascript:script = document.createElement('script');script.type = 'text/javascript';script.src = 'https://dl.dropboxusercontent.com/s/5f0ewzcwkh39pau/TESTE12.js?dl=0';document.getElementsByTagName("head")[0].appendChild(script);void(0);`],
  ...
  $.getScript("https://dl.dropboxusercontent.com/s/bytvle86lj6230c/resBalancer.js?dl=0" );void(0);
    ...
    },
    {
      ...
      scripts: [
        ["Noblagens", "https://icons.iconarchive.com/icons/iconfactory/sketchcons/32/smiley-icon.png", `javascript: var api = "https://api.codetabs.com/v1/proxy?quest=br.twstats.com/"; var twstats = "/index.php?page=ennoblements&live=live"; var url = api + game_data.world + twstats; var indexvillage = "index.php?page=village&id="; var indexplayer = "index.php?page=player&id="; var indextribe = "index.php?page=tribe&id="; var screenvillage = "&screen=info_village&id="; var screenplayer = "&screen=info_player&id="; var screenally = "&screen=info_ally&id="; var game = "/game.php?village="; $.get(url, function(data) { if ($(data).find(\"tr\").hasClass(\"r1\")) { table = $(data).find(\".widget\").eq(1).find(\"tr\"); } else { table = \"Ezen a szerveren most nincs élő foglalás.\"; } Dialog.show(\"foglalasok\", function(container) { $(`<table class=\"vis\" id=\"table\"><tbody></tbody></table>`).appendTo(container); $(table).appendTo(\"#table > tbody\"); }); $(\"#table\").find(\"a\").each(function(key,val) { var value = $(val).attr(\"href\"); if ($(val).is(`[href*=\"${indexvillage}\"]`)) { $(val).attr(\"href\", value.replace(indexvillage, game + game_data.village.id + screenvillage)); } if ($(val).is(`[href*=\"${indexplayer}\"]`)) { $(val).attr(\"href\", value.replace(indexplayer, game + game_data.village.id + screenplayer)); } if ($(val).is(`[href*=\"${indextribe}\"]`)) { $(val).attr(\"href\", value.replace(indextribe, game + game_data.village.id + screenally)); } }); }); void(0);`],
...
menu.appendChild(scrollHost);
  ...
  s.src = url;
  s.onload = () => console.log('Script carregado:', url);
  s.onerror = () => alert('Erro ao carregar o script!');
  ...
  window.location.href = url.replace('{game}', 'game.php');
  return;
}
